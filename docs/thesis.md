...
**UID** – User Identifier – уникальный идентификатор пользователя в Unix/Linux  
...
суперпользователя, обычно с использованием user namespaces и ограничений
доступа.  
...
**gVisor**, разработанный Google, – реализует собственную «виртуальную»
реализацию системных вызовов, фактически создавая второе, минимальное ядро,
работающее в пространстве пользователя. Процессы не получают прямого доступа к
системным вызовам хоста, все обращения к ядру фильтруются и обрабатываются
gVisor-ом. Такая модель изоляции обеспечивает гораздо более жёсткий контроль и
серьезно снижает вероятность выхода из контейнера, но и дает о себе знать – она
...
обычному пользователю, которому просто нужно «чтобы скрипт не затронул систему»,
...
контексте кратковременных изолированных процессов – это уже лишнее.

AppArmor использует другой подход. Это не контейнер, и не песочница, а скорее
...
ориентирован на запуск приложений от обычного пользователя, без привлечения
root. Это делает его особенно ценным для задач, где хочется жестко ограничить
процесс, но не вовлекать лишнюю сложность. Пространства имён, tmpfs, readonly
точки монтирования, ограничение доступа – всё явно прописывается при запуске.
...
# Настройка workflow
Инструменты статического анализа и линтеры – это возможность держать код в
порядке, не полагаясь каждый раз только на внимание разработчика. Когда проект
становится хоть немного сложнее, чем скрипт из пары функций, то начинают
появляться неочевидные ошибки и мелкие недочеты. Ручной контроль уже не спасает
– слишком много всего держать в голове.

Линтеры помогают выстроить границы: задать codestyle, обнаружить потенциальную
ошибку, обратить внимание на неочевидное поведение. Так как выловленная мелочь
при написании кода в будущем экономит кучу времени при отладке «гейзенбага».

Интеграция с CI (Continuous Integration) – это следующая ступень автоматизации.
Проверки становятся частью процесса: они срабатывают автоматически, не требуют
внимания, не дают забыть про качество, даже если разработчик торопится или
просто устал. Такой подход создает минимальный, но постоянный контроль.
...
- имя окружения (если не указано, выводится автоматически из имени исполняемого
  файла или программы);
- режим изоляции домашней директории (--private, --overlay);
- подключение сетевого пространства (--net);
...
возникает PermissionError, он логируется.

Дополнительный аргумент dbg_offset используется исключительно для отладочного
вывода – он визуально показывает глубину рекурсии в логах, добавляя отступы.
...
Листинг 1 - Пример минимальной конфигураций проекта
```
[project]
name = "wraplify"
version = "0.2.3"
