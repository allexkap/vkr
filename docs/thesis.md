...
с API  
**SSH** – Secure Shell – безопасный протокол удаленного доступа к системе  
...
**Монтирование** – Процесс привязки файловой системы к определенной директории.  
...
подготовленных контейнеров. Они реализуют работу с пространствами имён, cgroups
...
невозможна. Он может запускать образы в формате Docker, что делает его удобным
мостом между стандартной экосистемой и ограниченными средами исполнения.
Инструмент написан на Python и ориентирован в первую очередь на
переносимость.
...
лежат механизмы пространств имен и фильтрация системных вызовов, что позволяет
ограничивать взаимодействие приложений с системными ресурсами. Особенностью
Firejail является использование предопределенных профилей безопасности, которые
автоматически применяют политики изоляции для популярных программ (например,
браузеров, медиаплееров или офисных приложений). Это делает его удобным для
быстрой настройки среды без ручного конфигурирования.
...
основанный на механизмах пространств имен Linux. Его ключевая особенность –
...
запуска приложений в изолированных средах, что подтверждает его надежность и
...
сохраняя фокус на безопасности и легкости интеграции.
...
## Пространство имен UNIX Time Sharing
...
## Пространство имен Process Identifier
...
## Пространство имен Inter Process Communication
...
ограничения и в каком порядке. Но такой подход плохо масштабируется в рамках
...
использование – LXC рассчитан на долгоживущие контейнеры, где
важна устойчивость и контроль. В контексте кратковременных изолированных
процессов – его использование не оправдано.
...
плохо. Профили создаются отдельно, требуют установки и не меняются на в
рантайме. Его применение имеет смысл тогда, когда известно поведение
программы.
...
значительным объемом кода, что усложняет интеграцию в минималистичные решения.
...
ограничивает его применение в rootless-сценариях.
...
усложняется, и необходимость в разных сценариях требует хоть какой-то
структуры.
...
и запуском. Python оказался уместным выбором. Код на Python легко
читается, стандартная библиотека закрывает почти все базовые потребности – от
работы с путями и файлами до запуска процессов и логирования. Никакой
дополнительной сборки или компиляции не требуется, зависимости минимальны,
запуск возможен на большинстве современных Linux-систем без установки
дополнительных компонентов, Python чаще всего входит в стандартную поставку
дистрибутивов.

Это не самый производительный инструмент, но в этой задаче и не требуется
высокая производительность, так как это не CPU-bound задача. Гораздо
важнее простота изменений и прозрачность логики.
...
Изоляция обеспечивается за счет механизмов, предоставляемых ядром Linux, а
именно namespaces, cgroups, seccomp. Задача защиты пользовательского каталога от
удаления и изменения файлов и директорий, но в то же время предоставление
доступа к этим объектам, в рамках сохранения функциональности и
работоспособности большинства программ решается за счет OverlayFS. Работу с
этими низкоуровневыми механизмами берет на себя bubblewrap. Он позволяет
абстрагироваться от ручной настройки окружения и уделить внимание более
высокоуровневой логике. Но тем не менее он не создает дополнительных абстракций,
по-прежнему позволяя иметь точный контроль над окружениями. Так как bubblewrap
является очень легкой оберткой над механизмами ядра, он по определению не берет
на себя дополнительную работу, связанную с управлением настройками и содержимым
окружений. Разрабатываемое программное обеспечение, опираясь на bubblewrap,
выполняет конфигурационную работу, подгружая конфиги, анализируя параметры
командной строки и генерируя необходимые десятки параметров bwrap'у с передачей
...
Корневой каталог монтируется в режиме только для чтения (RO – Read Only), так
как на изменения файлов там у обычного пользователя и так нет прав. Домашняя
директория тоже монтируется в режиме RO, чтобы предотвратить любые изменения. Но
это отразится на работоспособности подавляющего большинства программ, которые
опираются на такую базовую возможность как запись в пользовательскую директорию.
Чтобы обойти это ограничение OverlayFS собирает финальную директорию из так
называемых слоев, где все кроме самого верхнего доступны лишь для чтения, а
верхний сохраняет в себе все изменения. Таким образом запущенное приложение в
изолированной среде по-прежнему может сохранять, изменять, удалять файлы и
директории, но эти изменения не будут видны за пределами его окружения. Также
стоит отдельно отметить возможность подключения некоторых директорий напрямую в
изолированное окружение, чтобы у процесса в этом окружении была возможность
контролируемо влиять на хостовую файловую систему.
...
эксплойтов, осуществляющих выход за пределы изолированного окружения.
...
1. The internals and the latest trends of container runtimes (2023) – URL:
...
1. Awesome Linux Containers – URL:
    https://github.com/Friz-zy/awesome-linux-containers
    (дата обращения 20.04.2025).
1. Awesome Containers – URL: https://github.com/pditommaso/awesome-containers
    (дата обращения 20.04.2025).
1. Containers – URL: https://github.com/containers/ (дата обращения 20.04.2025).
1. What is Docker? – URL: https://docs.docker.com/get-started/docker-overview/
    (дата обращения 20.04.2025).
1. Базовые возможности LXD – системы контейнеров в Linux – URL:
    https://habr.com/ru/articles/496492/ (дата обращения 20.04.2025).
1. Rootless containers using Podman – URL:
    https://www.redhat.com/en/blog/rootless-containers-podman
    (дата обращения 01.04.2025).
1. What is gVisor? – URL: https://gvisor.dev/docs/ (дата обращения 20.04.2025).
1. Firecracker – Lightweight Virtualization for Serverless Computing – URL:
    https://aws.amazon.com/blogs/aws/firecracker-lightweight-virtualization-for-serverless-computing
    (дата обращения 20.04.2025).
1. runc – URL: https://github.com/opencontainers/runc
    (дата обращения 20.04.2025).
1. An introduction to crun, a fast and low-memory footprint container runtime –
    URL: https://www.redhat.com/en/blog/introduction-crun
    (дата обращения 20.04.2025).
1. Youki User and Developer Documentation – URL:
    https://youki-dev.github.io/youki/youki.html (дата обращения 20.04.2025).
1. udocker – URL: https://github.com/indigo-dc/udocker
    (дата обращения 20.04.2025).
1. nsjail – URL: https://nsjail.dev (дата обращения 20.04.2025).
1. Firejail – URL: https://wiki.archlinux.org/title/Firejail
    (дата обращения 20.04.2025).
1. Bubblewrap – URL: https://wiki.archlinux.org/title/Bubblewrap
    (дата обращения 20.04.2025).
1. namespaces(7) – URL: https://man.archlinux.org/man/namespaces.7
    (дата обращения 21.04.2025).
1. cgroups(7) – URL: https://man.archlinux.org/man/cgroups.7
    (дата обращения 23.04.2025).
1. seccomp(2) – URL: https://man.archlinux.org/man/seccomp.2
    (дата обращения 23.04.2025).
1. selinux(8) – URL: https://man7.org/linux/man-pages/man8/selinux.8.html
    (дата обращения 23.04.2025).
1. Containers and Threads from the POV of `clone` – URL:
    https://minin.tech/posts/containers-and-threads-from-the-pov-of-clone/
    (дата обращения 21.04.2025).
1. user_namespaces(7) – URL:
    https://man.archlinux.org/man/core/man-pages/user_namespaces.7
    (дата обращения 21.04.2025).
1. mount_namespaces(7) – URL:
    https://man.archlinux.org/man/core/man-pages/mount_namespaces.7
    (дата обращения 21.04.2025).
1. utc_namespaces(7) – URL:
    https://man.archlinux.org/man/core/man-pages/utc_namespaces.7
    (дата обращения 21.04.2025).
1. pid_namespaces(7) – URL:
    https://man.archlinux.org/man/core/man-pages/pid_namespaces.7
    (дата обращения 21.04.2025).
1. network_namespaces(7) – URL:
    https://man.archlinux.org/man/core/man-pages/network_namespaces.7
    (дата обращения 21.04.2025).
1. ipc_namespaces(7) – URL:
    https://man.archlinux.org/man/core/man-pages/ipc_namespaces.7
    (дата обращения 21.04.2025).
1. clone(2) – URL: https://man.archlinux.org/man/clone.2
    (дата обращения 21.04.2025).
1. unshare(2) – URL: https://man.archlinux.org/man/unshare.2
    (дата обращения 21.04.2025).
1. The Python Standard Library – URL:
    https://docs.python.org/3.13/library/index.html (дата обращения 22.04.2025).
1. CVE-2016-5195 – URL: https://nvd.nist.gov/vuln/detail/cve-2016-5195
    (дата обращения 23.04.2025).
1. CVE-2019-5736 – URL: https://nvd.nist.gov/vuln/detail/cve-2019-5736
    (дата обращения 23.04.2025).
1. Common questions – URL: https://pypi.org/help/ (дата обращения 24.04.2025).
1. Writing your pyproject.toml – URL:
    https://packaging.python.org/en/latest/guides/writing-pyproject-toml/
    (дата обращения 24.04.2025).
1. Python Packaging Authority – URL: https://www.pypa.io/en/latest/
    (дата обращения 24.04.2025).
1. Package Formats – URL:
