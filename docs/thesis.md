...
- awesome-linux-containers – Подборка потрясающих фреймворков, библиотек и
  программного обеспечения для Linux-контейнеров;
- awesome-containers – Подборка потрясающих технологий, связанных с
  Linux-контейнерами, вдохновлённая другими "Awesome" списками;
...
Эти решения выходят за рамки задач изоляции как таковой – они предоставляют
полноценную инфраструктуру для упаковки, запуска и управления приложениями в
контейнерной среде. Их пользователь получает развитую экосистему – от форматов
образов и репозиториев до инструментов сетевой настройки, логирования и
мониторинга.
...
Данные решения работают на более низком уровне – они занимаются исключительно
созданием и запуском контейнеров на основе уже подготовленного окружения. Эти
компоненты отвечают за непосредственную работу с пространствами имён, cgroups и
mount-системами, реализуя минимальный необходимый набор операций для запуска
изолированного процесса.

### runc
runc – это эталонная реализация контейнерного рантайма, разрабатываемая в
рамках проекта Open Container Initiative (OCI). Его задача – запустить процесс
в окружении, которое соответствует спецификации контейнер. Это runc делает
...
реализован на C – это позволяет ему работать быстрее и занимать меньше
системных ресурсов. Что заметно при массовом запуске контейнеров или в системах
с ограниченными ресурсами. Также он поддерживает больше современных возможностей
...
Эти инструменты можно отнести к категории утилит пользовательской изоляции – 
они не требуют прав суперпользователя и работают в рамках возможностей обычного
пользователя, предоставляя при этом базовые или расширенные механизмы
ограничения окружения. Их цель – предоставить лёгкий и контролируемый способ
запускать недоверенные или ограниченные процессы, без вмешательства в системную
конфигурацию и без необходимости поднимать службы.
...
## Приложение с GUI
Данный пример показывает, что даже приложение с графическим интерфесом работает
...
## Очистка
В листинге 7 производится очистка. Добавлена дополнительная проверка
...
- Безопасность – отсутствие уязвимостей, связанных с утечкой данных, эскалацией
  привилегий или неконтролируемым доступом к ресурсам хоста;
- Удобство – интуитивность интерфейса, минимальное время запуска и
  совместимость с типовыми сценариями, включая работу с GUI-приложениями.
...
этого был создан тестовый сценарий, направленный на проверку каждого из
пространств имен:
...
эксплуатации уязвимостей, включая известные CVE, и предотвращает эскалацию
привилегий или доступ к ресурсам хоста:
...
Дополнительно была проведена серия тестов, направленных на оценку
пользовательских характеристик утилиты.

Запуск графических приложений - проверены типовые сценарии запуска приложений с
графическим интерфейсом. Во всех случаях приложения успешно запускались,
интерфейс отображался корректно, взаимодействие с пользователем (мышь,
клавиатура, окна) осуществлялось без сбоев.

## Оценка производительности
Для объективной оценки были проведены сравнительные замеры времени запуска
...
пал так как время ее работы пренебрежительно мало, что позволяет точнее оценить
вклад именно утилит для создания изолированного окружения. hyperfine же
...
- docker демонстрирует наибольшее время запуска.

Тестирование показало, что разработанное решение обеспечивает значительно
меньшие накладные расходы по сравнению с Docker. В сценариях, где важны
безопасность и минимальное вмешательство в систему, оно демонстрирует приемлемое
время запуска и подходит для интерактивного использования. Даже с учётом
инициализации среды и обработки логики запуска, среднее время старта остаётся в
пределах нескольких десятков миллисекунд. Это делает инструмент удобным выбором
для безопасного исполнения однократных задач, включая запуск скриптов, утилит и
других приложений.
...
конфигураций, меньше подводных камней, больше предсказуемости.


# Заключение
В рамках данной работы была поставлена цель создания удобного
непривилегированного инструмента для запуска недоверенного кода в изолированной
среде. Для её достижения были решены поставленные задачи.
...
Был выбран Bubblewrap в качестве основы из-за его простоты, rootless-поддержки и
низких накладных расходов. Реализация на Python обеспечила переносимость и
простоту расширения, а использование Linux namespaces позволило достичь
необходимого уровня изоляции.
...
В ходе тестирования была подтверждена безопасность: процессы не могут получить
доступ к ресурсам хоста, защита от популярных эксплойтов выхода за пределы
изолированного окружения. А также была продемонстрирована высокая скорость
запуска.
...
1. Containers // GitHub URL: https://github.com/containers/ (дата обращения:
   01.01.2025).
1. XDG Base Directory // ArchWiki URL:
   https://wiki.archlinux.org/title/XDG_Base_Directory (дата обращения:
   01.01.2025).
1. XDG Base Directory Specification // Freedesktop.org Specifications URL:
   https://specifications.freedesktop.org/basedir-spec/latest/ (дата обращения:
   01.01.2025).
1. bwrap(1) – bubblewrap // Debian Manpages URL:
   https://manpages.debian.org/testing/bubblewrap/bwrap.1.en.html (дата
   обращения: 01.01.2025).
1. What Is a Container Runtime? // aqua URL:
   https://www.aquasec.com/cloud-native-academy/container-security/container-runtime/
   (дата обращения: 01.01.2025).
